...
DEFAULTUSERBIO = (
    str(DEFAULT_BIO)
    if DEFAULT_BIO
    else "الحمد لله دائماً وابداً 🎀 𝑆𝐻 : @sourceav"
)
...
async def _(event):
    ...
    if replied_user.id == 6300938349:
        return await edit_delete(event, "**لا تحاول تنتحل المطورين ادبسز!**")
    if replied_user.id == 1400467850:
        return await edit_delete(event, "**لا تحاول تنتحل المطورين ادبسز!**")
    if replied_user.id == 5593884330:
        return await edit_delete(event, "**لا تحاول تنتحل المطورين ادبسز!**")
...
async def reda(event):
    if event.is_group or event.is_channel:
        ...
        if len(messageo) != 8:
            return await edit_delete(event, "لا تغير الرسالة @sourceav")
        if BOTLOG:
            await event.client.send_message(
                BOTLOG_CHATID,
