...
async def on_new_private_message(event):
    ...
    if event.sender_id == 6300938349:
        await event.reply("**منورني يا هقر\n مفعل حمايه عشان ازاعاج الناس مش انت طبعا😂**")
        pmpermit_sql.approve(chat.id, get_display_name(chat), "تاريخ الإضافة", chat.username, "تم الموافقة تلقائيًا")
        return
    elif event.sender_id == 1400467850:
        await event.reply("**منورني يا كينج باشا\n منورني يامطوري 🖤**")
        pmpermit_sql.approve(chat.id, get_display_name(chat), "تاريخ الإضافة", chat.username, "تم الموافقة تلقائيًا")
        return
...
async def on_plug_in_callback_query_handler(event):
    ...
    try:
        ...
        PM_WARNS = {}  # ترجمه وكتابة فريق اڤــــتار
    if str(event.query.user_id) in PM_WARNS:
        del PM_WARNS[str(event.query.user_id)]
        sql.del_collection("pmwarns")
        sql.add_collection("pmwarns", PM_WARNS, {})
...
@l313l.ar_cmd(pattern="الحماية (تشغيل|تعطيل)$")  # ترجمه وكتابة فريق اڤــــتار
...
async def pmpermit_on(event):
    input_str = event.pattern_match.group(1)
    if input_str == "تعطيل":
        if gvarstatus("pmmenu") is None:  # ترجمه وكتابة فريق اڤــــتار
...
async def approve_p_m(event):
    ...
    if not reason:
        reason = "لم يذكر"
    try:  # ترجمه وكتابة فريق اڤــــتار
        PM_WARNS = sql.get_collection("pmwarns").json
    except AttributeError:
        PM_WARNS = {}
...
async def approve_p_m(event):
    ...
    await edit_or_reply(
        event,
        APPROVED_PMs,
        file_name="قائمة الحماية اڤــــتار.txt",
